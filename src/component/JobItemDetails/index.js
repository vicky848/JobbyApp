...
class JobItemDetails extends Component {
  ...
  getFormattedSkillData = data => ({
    companyLogoUrl: data.compnay_logo_url,
    companyWebsiteUrl: data.company_website.logo,
    employmentType: data.employment_type,
    id: data.id,
    jobDescription: data.job_description,
  ...
  getJobItem = async () => {
    ...
    if (response.ok === true) {
      ...
      const updatedSkillData = data.similar_job.map(eachSimilarJob =>
    ...
    return (
      <div className="main-job-item-container">
        <div className="job-item-container">
          ...
          <h className="skill-heading">Skills</h>
          <ul className="skill-container">
            {skills.map(eachSkill => (
              <SkillsCard key={eachSkill.id} skillsDetails={eachSkill} />
            ))}
          </ul>
  ...
  renderFailureView = () => (
    <div className="render-failure-view">
      <img
        ...
        alt="failure view"
        className="failure-view-logo"
      />
      <h1 className="failure-heading">Oops! Something Went Wrong</h1>

      <p className="faliure-desc">
  ...
  renderJobView = () => {
    ...
    switch (apiStatus) {
      case apiStatusConstants.success:
        return this.renderJobItamDetails()
